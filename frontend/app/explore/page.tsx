...
function GenrePill({
  ...
  return (
    <button
      onClick={onClick}
      className={`h-[40px] px-[20px] rounded-full font-sans text-[14px] leading-[30px] transition-all duration-200 ${
        active
...
function TrackRow({
  ...
  return (
    <Link href={`/track/${track.id}`}>
      <div className="flex items-center gap-[20px] py-[16px] px-[20px] rounded-[14px] hover:bg-white/5 transition-colors group cursor-pointer">
        <span className="font-sans text-[16px] text-white/50 w-[30px] text-center">{index}</span>
        <div className="w-[56px] h-[56px] rounded-[10px] overflow-hidden shrink-0">
        ...
        <span className="font-sans text-[14px] text-white/50 w-[80px] text-center">{track.genre || '—'}</span>
        <span className="font-sans font-medium text-[14px] text-white w-[100px] text-right">
...
export default function ExplorePage() {
  ...
  return (
    <div className="bg-brand-bg min-h-screen relative overflow-hidden">
      {/* ===== HERO SECTION ===== */}
      <section className="relative w-full h-[597px]">
        <div className="absolute inset-0 bg-brand-bg" />

        {/* Radial glow - blue */}
        <div className="absolute right-[10%] top-[60px] w-[407px] h-[407px] pointer-events-none">
          <div
        ...
        {/* Radial glow - pink */}
        <div className="absolute right-[2%] top-[60px] w-[407px] h-[407px] pointer-events-none">
          <div
            className="absolute inset-0 rounded-full opacity-50"
        ...
        <div className="relative z-10 flex flex-col items-center pt-[168px]">
          ...
          <div className="w-[238px] h-[62px] mb-[45px]">
          ...
          <h1 className="font-heading text-[46px] text-white text-center w-[848px] mb-[14px]">
            Explore Music
          </h1>
          <p className="font-sans text-[16px] text-white text-center leading-[30px]">
      ...
      {/* ===== SEARCH & FILTERS ===== */}
      <section className="relative z-10 px-[164px] -mt-[20px]">
        <div className="w-[1172px]">
          {/* Search Bar */}
          <form onSubmit={handleSearch} className="mb-[24px]">
            <div className="relative w-full h-[73px] rounded-[39px] border-2 border-white bg-brand-bg overflow-hidden flex items-center">
              <Search className="absolute left-[24px] w-5 h-5 text-white/50" />
              <input
                type="text"
                placeholder="Search tracks, artists..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full h-full bg-transparent text-white font-sans text-[16px] leading-[30px] pl-[56px] pr-[140px] focus:outline-none placeholder-white/40"
              />
              <button
                type="submit"
                className="absolute right-[8px] btn-gradient-sm !py-[14px] !px-[30px] text-[16px]"
              >
                Search
              </button>
            </div>
          ...
          <div className="flex items-center gap-[14px]">
            <div className="relative">
              <select
                ...
                className="appearance-none h-[40px] px-[20px] pr-[40px] rounded-full border border-white bg-brand-bg text-white font-sans text-[14px] leading-[30px] focus:outline-none cursor-pointer"
              >
                {sortOptions.map((opt) => (
      ...
      <section className="px-[164px] mt-[80px]">
        <div className="w-[1172px]">
          ...
          {/* Tracks Display */}
          {loading ? (
            <div className={viewMode === 'grid' ? 'grid grid-cols-3 gap-x-[40px] gap-y-[41px]' : 'flex flex-col'}>
              {[...Array(viewMode === 'grid' ? 9 : 6)].map((_, i) => (
                <div key={i} className={viewMode === 'grid' ? 'w-[364px]' : ''}>
                  {viewMode === 'grid' ? (
            ...
            <div className="flex flex-col">
              {/* List Header */}
              <div className="flex items-center gap-[20px] py-[12px] px-[20px] border-b border-white/10 mb-[8px]">
